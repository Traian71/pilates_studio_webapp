...
interface Plan {
  ...
  price: number;
  validity_days?: number; // Added from plans_mat schema
  description?: string;  // Added from plans_mat schema
  type?: 'mat' | 'reformer'; // Keep if used for display, though data comes from specific table
}
...
const SelectMatPlanPage = () => {
  ...
  useEffect(() => {
    const fetchPlans = async () => {
      setLoading(true);
      // Fetch only mat plans
      const { data, error } = await supabase
        .from('plans_mat') // Changed to plans_mat
        .select('id, name, session_count, price, validity_days, description');
      
      if (error) {
        console.error('Error fetching mat plans:', error);
        setError('Failed to load mat plans. Please try again later.');
      } else {
  ...
  const handlePlanActivation = async (paymentMethod: 'stripe' | 'cash') => {
    ...
    if (selectedPlan.validity_days) {
      const expiryDate = new Date(currentDate.setDate(currentDate.getDate() + selectedPlan.validity_days));
      endDate = expiryDate.toISOString().split('T')[0]; // YYYY-MM-DD
    }

    const subscriptionData = {
      client_id: user.id,
      plan_mat_id: selectedPlan.id,
      plan_reformer_id: null,
      start_date: startDate,
      end_date: endDate,
      sessions_total: selectedPlan.session_count,
      classes_remaining: selectedPlan.session_count,
      price_paid: selectedPlan.price,
      is_active: true,
      payment_status: paymentMethod === 'stripe' ? 'pending' : 'paid',
      // transaction_id could be set after successful Stripe payment
    };

    try {
      // First, ensure client exists in 'clients' table as client_subscriptions has FK to it
      // This step might be redundant if your signup process guarantees a 'clients' entry
      // or if you handle it differently (e.g., a trigger on auth.users insert).
      // For safety, we include an upsert here if not already handled.
      const { error: clientUpsertError } = await supabase
        .from('clients')
        .upsert({ 
            id: user.id, 
            first_name: user.user_metadata.first_name || userDetails.firstName, 
            last_name: user.user_metadata.last_name || userDetails.lastName,
            // phone: user.user_metadata.phone || userDetails.phone // if phone is in clients table
        }, { onConflict: 'id' });

      if (clientUpsertError) {
        console.error('Error ensuring client entry:', clientUpsertError);
        // Decide if this is a critical error. If client_subscriptions.client_id requires an existing clients row, it is.
        // setError(`Failed to prepare client record: ${clientUpsertError.message}`);
        // return false;
      }

      // Now, insert into client_subscriptions
      ...
      console.log('Client subscription created successfully for user:', user.id, 'Mat Plan ID:', selectedPlan.id);
      setError(null); 
      return true;
    } catch (e: any) {
      setError(`Failed to activate plan: ${e.message}`);
      console.error('Error creating client subscription:', e);
      return false;
    }
  };
      ...
      case 0: // Select Plan
        ...
        return (
          <div className="w-full">
            ...
            {plans.length === 0 ? (
              <p className="text-center text-gray-600">Nu sunt disponibile momentan abonamente mat. Vă rog să reveniți mai târziu.</p>
            ) : (
              <div className="flex justify-center gap-4 mb-8 flex-wrap">
                {plans.map((plan) => (
                  <div key={plan.id} className="bg-white p-4 rounded-lg shadow-md border border-gray-200 hover:shadow-lg transition-shadow cursor-pointer w-[260px] md:w-[280px]" onClick={() => { setSelectedPlan(plan); nextStep(); }}>
                    <div>
            ...
            <form onSubmit={(e: FormEvent) => { e.preventDefault(); nextStep(); }} className="space-y-4">
              <div>
                ...
                <Input 
                  ...
                  className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-deep-teal focus:ring focus:ring-deep-teal/50 focus:ring-opacity-50 text-sm" 
                  required 
                />
              </div>
              <div>
        ...
        return (
          <div className="w-full max-w-2xl mx-auto">
            ...
            <div className="bg-slate-50 p-4 rounded-md mb-6 border border-slate-200">
              ...
              <p className="text-xl"><span className="font-bold">{selectedPlan.name}</span></p>
            ...
            <div className="flex justify-between mt-8">
              ...
              <Button onClick={nextStep} className="bg-foreground text-background hover:bg-foreground/90 focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-ring">Plată</Button>
            </div>
          </div>
        );
      case 3: // Payment
        ...
        if (error) { // Display error if activation failed
          return (
            <div className="w-full max-w-lg mx-auto text-center">
              <h2 className="text-2xl md:text-3xl font-semibold text-red-600 mb-6">Activation Failed</h2>
              <p className="text-gray-700 mb-4">{error}</p>
              <p className="text-gray-700 mb-8">Încercă din nou sau contactează sprijinul.</p>
              <Button 
                variant="outline" 
          ...
          <div className="w-full max-w-lg mx-auto text-center">
            ...
            {selectedPlan && (
              <div className="bg-slate-50 p-6 rounded-md mb-8 border border-slate-200 text-left">
                <h3 className="text-lg font-medium mb-4 font-inter text-[#E57F84]">Detaliile planului tău:</h3>
                <p className="text-gray-700"><strong>Nume Plan:</strong> {selectedPlan.name}</p>
...
export default SelectMatPlanPage;
